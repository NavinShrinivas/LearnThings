...
//Lets consider that imaginary numbers are store ad string 
pub trait RealCoords{
    fn dist_from_origin(self:&Self)->f32; //No default
}

pub trait PrintPretty {
    fn print_pretty(self:&Self){
        println!("Default print pretty, no specific impl from objecy");
    }
}
...
impl<T:Debug,U:Debug> PrintPretty for Coords<T,U>{
    fn print_pretty(self:&Self) {
        println!("( {:#?},{:#?} )",self.x,self.y);
    }
}

impl PrintPretty for dyn Debug{}
